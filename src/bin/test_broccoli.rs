...
fn concat(files:&mut [UnlimitedBuffer],
  ...
  let mut bytealigned_ibuffer = Vec::with_capacity(bs + BYTE_ALIGNED_START_BLOCK_WINDOW_22.len() + BYTE_ALIGNED_END_BLOCK.len());
  ...
  {
    bytealigned_ibuffer.write_all(BYTE_ALIGNED_START_BLOCK_WINDOW_22).unwrap();
    ...
    for (file, brotli) in files.iter_mut().zip(byte_aligned_brotli_files.iter_mut()) {
      ...
      // test decompressing the indiviudal block
      {
        ...
        block_ibuffer.write_all(BYTE_ALIGNED_START_BLOCK_WINDOW_22).unwrap();
    ...
    bytealigned_ibuffer.write_all(BYTE_ALIGNED_END_BLOCK).unwrap();
          ...
          loop {
            ...
            match input.read(&mut brotli_ibuffer[..]) {
              ...
              Ok(cur_read) => {
                ...
                loop {
                  match bro_cat_li.stream(&brotli_ibuffer[..cur_read], &mut ioffset,
...
fn concat_many_subsets(files:&mut [UnlimitedBuffer],
  ...
  for plan_bs in test_plans.iter() {
    ...
    for index in 0..(brotli_files.len() - core::cmp::min(plan_bs.0 - 1, files_len)) {
      ...
      let bytealign_subset = &mut bytealign_brotli_files[index..core::cmp::min(index+plan_bs.0, files_len)];
      concat(file_subset, brotli_subset, bytealign_subset, window_override, plan_bs.1);
    }
  }
}
...
fn test_appendonly_twice_fails() {
  ...
  let buffers = files.iter_mut().zip(ufiles.iter_mut()).zip(byte_aligned_ufiles.iter_mut());
...
fn test_concat() {
    ...
    for option in options.iter_mut().skip(3) {
        ...
        let mut first = true;
        for ((src, dst), align_dst) in files.iter_mut().zip(ufiles.iter_mut()).zip(byte_aligned_ufiles.iter_mut()) {
    ...
    for (index, ((src, dst), align_dst)) in files.iter_mut().zip(ufiles.iter_mut()).zip(byte_aligned_ufiles.iter_mut()).enumerate() {
    ...
    concat_many_subsets(&mut files, &mut ufiles, &mut byte_aligned_ufiles, None);
    concat_many_subsets(&mut files, &mut ufiles, &mut byte_aligned_ufiles, Some(28)); // FIXME: make this 28
}
